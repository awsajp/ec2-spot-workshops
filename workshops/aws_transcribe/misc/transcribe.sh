...
ECR_REPO="000474600478.dkr.ecr.us-east-1.amazonaws.com/ecr-demo-scan/web"
GIT_SHA=$(git rev-parse --short=7 master) 
echo "GIT_SHA = $GIT_SHA"
docker build --no-cache  -t $ECR_REPO:$GIT_SHA .
...
True :, images will be scanned after being pushed
False:  images will not be scanned unless a scan is manually started 
To create a repository configured for scan on push (AWS CLI)
aws ecr create-repository --repository-name name --image-scanning-configuration scanOnPush=true --region us-east-1
Configure an existing repository to scan on push 

jp:~/environment/ec2-spot-workshops/workshops/ecr-image-scanning (master) $ aws ecr put-image-scanning-configuration --repository-name ecs-spot-workshop/webapp --image-scanning-configuration scanOnPush=true --region us-east-1
{
...
aws ecr start-image-scan --repository-name ecr-demo-scan/web --image-id imageTag=$GIT_SHA --region us-east-1
aws ecr start-image-scan --repository-name ecs-spot-workshop/web --image-id imageTag=latest --region us-east-1
aws ecr start-image-scan --repository-name ecs-spot-workshop/webapp --image-id imageTag=latest --region us-east-1

jp:~/environment/ec2-spot-workshops/workshops/ecr-image-scanning (master) $ aws ecr start-image-scan --repository-name ecs-spot-workshop/webapp --image-id imageTag=latest --region us-east-1
{
    "registryId": "000474600478",
    "repositoryName": "ecs-spot-workshop/webapp",
    "imageId": {
        "imageDigest": "sha256:ccda837952143961d89dd29c7b78aacde1917de4a87369c5f20d33b18599afa2",
        "imageTag": "latest"
    },
    "imageScanStatus": {
        "status": "IN_PROGRESS"
...
aws ecr describe-image-scan-findings --repository-name $ECR_REPO --image-id imageTag=tag_name --region us-east-1

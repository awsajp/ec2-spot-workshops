...
def createLaunchTemplate():
  ...
  try:
    ...
    if status_code == 200:
      ...
      pprint("createLaunchTemplate failed with state={} message={}".format(state, str(response)))
      return state, str(response)
...
def CleanupClusterResources():
  try:
    ...
    pprint("CleanupClusterResources starting finding all the Instances with TagKey={} TagValue={}".format(TagKey, TagValue))
    response = ec2client.describe_instances(Filters=[{'Name': 'tag:'+TagKey,'Values':[TagValue]}, {'Name': 'instance-state-name', 'Values': ['running']} ])
    #pprint(response)
...
def returnFromLambda(event, context, message):
  ...
  if 'RequestType' in event.keys():
    responseData = {}
    responseData['Data'] = 'Dummy'
    cfnresponse.send(event, context, cfnresponse.SUCCESS, responseData,"Dummy")
  return {
    'statusCode': 200,
    ...
    if event['RequestType'] == "Create":
      ...
      try:
        ...
        if 'Item' in response.keys():
          ...
          pprint("Adding launchTemplateId={} launchTemplateName={} launchTemplateVersion={}".format(launchTemplateName, launchTemplateId, launchTemplateVersion))
          response = InstanceTable.put_item(
            Item={
      ...
      #createSnapshots(InstanceId) # for testing purpose
    elif event['RequestType'] == "Update":
      InstanceId = event['InstanceId']
      #updateInstanceIdInDynamoDB(InstanceId) # for testing purpose
    else:

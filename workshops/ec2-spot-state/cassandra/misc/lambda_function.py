...
def updateInstanceDataInDynamoDB(InstancesDescription):
...
def updateInstanceIdInDynamoDB(InstanceId):
    print("Running updateInstanceIdInDynamoDB for InstanceId={}".format(InstanceId))
...
def updateExistingClusterinDynamoDB(ExistingClusterInstanceTag):
    ...
    response = ec2client.describe_instances(Filters=[{'Name': 'tag:'+key,'Values':[value]}])
    #pprint(response)
    InstancesDescription= response['Reservations'][0]['Instances']
    pprint("number of instances={}".format(len(InstancesDescription)))
    updateInstanceDataInDynamoDB(InstancesDescription)
    ...
    if 'RequestType' in event.keys():
        if event['RequestType'] == "Create":
            ...
            if UseExistingCluster == "YES":
                ...
                updateExistingClusterinDynamoDB(ExistingClusterInstanceTag)
            else:

...
if __name__ == '__main__':
    ...
    """
    f= open("templates/index3.html","a")
    try:
      instanceId = requests.get("http://169.254.169.254/latest/meta-data/instance-id")
      f.write("<li>My instance_id = {}</li>".format(instanceId.text))
      instanceType = requests.get("http://169.254.169.254/latest/meta-data/instance-type")
      f.write("<li>My instance_id = {}</li>".format(instanceType.text))
      privateIp = requests.get("http://169.254.169.254/latest/meta-data/local-ipv4")
      f.write("<li>My instance_id = {}</li>".format(privateIp.text))
      publicIp = requests.get("http://169.254.169.254/latest/meta-data/public-ipv4")
      f.write("<li>My instance_id = {}</li>".format(publicIp.text))
      AZ = requests.get("http://169.254.169.254/latest/meta-data/placement/availability-zone")
      f.write("<li>My instance_id = {}</li>".format(AZ.text))
      instanceId = requests.get("http://169.254.169.254/latest/meta-data/instance-id")
      f.write("<li>My instance_id = {}</li>".format(instanceId.text))
      instanceId = requests.get("http://169.254.169.254/latest/meta-data/instance-id")
      f.write("<li>My instance_id = {}</li>".format(instanceId.text))      
      ...
      f.write("<li>Oops !!! Failed to access my instance  metadata with error = {}</li>".format(inst))
